/* Write an openmp pgm to sort array of n elements using both seq & pellel merge sort (usiong the sectin directive)
 record & print diff i execn time btw 2 meth's
 */
...
void mergeSortSequential(int arr[], int left, int right)
{
    if (left < right)
    {
        ...
        mergeSortSequential(arr, left, mid-1);
        mergeSortSequential(arr, mid , right);
...
int main()
{
    ...
    double time_seq = omp_get_time() - start_time;
